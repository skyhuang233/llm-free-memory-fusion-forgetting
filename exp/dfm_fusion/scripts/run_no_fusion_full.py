"""Run the No-Fusion ablation baseline: 3 runs x 10 conversations.
No fusion is performed -- memories are stored/retrieved individually.
Uses only answer-generation LLM calls (zero fusion cost).
Saves results to dfm_fusion/results/no_fusion/.
"""
...
def main():
    ...
    output_dir = Path("dfm_fusion/results/no_fusion")
    ...
    log.info(f"Starting No-Fusion baseline: {num_runs} runs x {len(conversations)} conversations")
    t_total = time.time()
